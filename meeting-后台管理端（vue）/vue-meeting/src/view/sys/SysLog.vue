  ...
  <div style="margin-top: 15px">
    ...
    <div class="operation-container">
      ...
      <div style="margin-left:auto">
        <el-input v-model="keywords" prefix-icon="el-icon-search" size="small" placeholder="请输入模块名或描述" style="width:200px" @keyup.enter.native="searchLogs" />
        <el-button type="primary" size="small" icon="el-icon-search" style="margin-left:1rem" @click="searchLogs">
          搜索
        </el-button>
        <el-button type="danger" size="small" icon="el-icon-delete" :disabled="this.logIdList.length == 0" @click="isDelete = true">
          批量删除
        </el-button>
      </div>
    </div>
    ...
    <el-table @selection-change="selectionChange" v-loading="loading" :data="logList">
      ...
      <el-table-column label="操作" align="center" width="150">
        <template slot-scope="scope">
          <el-button size="mini" type="text" slot="reference" @click="check(scope.row)"> <i class="el-icon-view" /> 查看 </el-button>
...
export default {
  ...
  methods: {
    ...
    listLogs() {
      this.axios
        .get('/api/log/admin/operation/logs', {
          params: {
        ...
        .then(data => {
          this.logList = data.obj?.recordList
          this.count = data.obj?.count
          this.loading = false
        })
    },
    deleteLog(id) {
      ...
      this.axios.delete('/api/log/admin/operation/logs', param).then(({ data }) => {
        if (data.flag) {
          this.$notify.success({
            title: '成功',
